...
void main() {
  ...
  test(
    'buffers collisions and drains them into ECS events on frameStart',
    () async {
      final world = _FakeWorld();
      final app = App()..addPlugin(PhysicsPlugin(world));
      app.start();
  ...
  test('a collision fired during frame N physics surfaces in frame N+1 '
      '(async stream latency)', () async {
    // Async collisions arrive on the next frame.
    final world = _FakeWorld();
    final app = App()..addPlugin(PhysicsPlugin(world));
  ...
  test(
    'shutdown cancels the physics subscription and clears pending events',
    () async {
      final world = _FakeWorld();
      final app = App()..addPlugin(PhysicsPlugin(world));
      app.start();
